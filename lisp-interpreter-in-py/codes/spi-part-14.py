...
class Var(AST):
    ...
    def __str__(self):
        return "<{class_name}(var='{type}')>".format(
...
class VarDecl(AST):
    ...
    def __str__(self):
        return "<{class_name}(var='{name}', type='{type}')>".format(
            class_name=self.__class__.__name__,
            name=self.var_node,
            type=self.type_node
        )
...
class Parser(object):
    ...
    def declarations(self):
        """declarations : VAR (variable_declaration SEMI)+
                        | (PROCEDURE ID (LPAREN formal_parameter_list PRAREN)? SEMI block SEMI)*
                        | empty
        """
        declarations = []
        while True:
